...
export async function get_semesters(driver: ThenableWebDriver) {
  ...
  try {
    ...
    const semester_elements = await semester_element.findElements(
      By.xpath(".//option")
    );
...
export async function get_major_codes(driver: ThenableWebDriver) {
  ...
  try {
    ...
    const subject_elements = await subject_element.findElements(
      By.xpath(".//option")
    );
...
async function get_class_info(course_element: WebElement): Promise<string> {
  const description_element = await course_element.findElement(
    By.className("section__content")
  );
...
export async function get_classes(
  ...
  try {
    ...
    const [semester_elements, subject_element, search_button_element] =
      await Promise.all([
    ...
    const course_elements = await driver.wait(
      until.elementsLocated(By.className("result result--group-start"))
    );
    ...
    for (let course_element of course_elements) {
      ...
      const content_element = await driver.wait(
        until.elementLocated(
          By.className("panel panel--2x panel--kind-details panel--visible")
        )
      );
